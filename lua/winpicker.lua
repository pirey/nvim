...
function M.pick()
  ...
  -- Filter windows by buftype and filetype
  local filtered_wins = {}
  ...
  for i, win in ipairs(filtered_wins) do
    ...
    win_map[letter] = { main_win = win, float_win = float_win }
  end
  ...
  -- Jump to window if valid
  local target_win = win_map[input]
  if target_win then
    vim.api.nvim_set_current_win(target_win.main_win)
    vim.api.nvim_echo({}, false, {}) -- clear message
  else
    vim.api.nvim_echo({{"No window for input: " .. input, "ErrorMsg"}}, false, {})
  end
end
